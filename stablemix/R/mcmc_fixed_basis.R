#' Metropolis-Hastings algorithm for Gaussian Density basis stable mixture model
#' @description Main Metropolis-Hastings loop to generate posterior samples for
#' stable-mixture model when using Gaussian density kernel function basis.
...
#' @param knot_coord n-knot x 2 matrix of knot location coordinates for Gaussian
...
mcmc_fixed_basis <-
  ...
  nloc <- nrow(obs_coord)
  D <- calc_dist(knot_coord, obs_coord)
  knot_rng <- c(max(apply(D, 2, min))/2, max(calc_dist(knot_coord, knot_coord)))
  obs_rng <- max(D)/2
  win_len <- min(2*nobs, n_iter, 50)            # Window for acceptance rates and prop vars
  parnames <- c("alpha", "theta","tau")

  if(!is.null(mar_gp_which)){
    if(is.null(clusters)){
      km <- kmeans(obs_coord, floor(nrow(obs_coord)/20))
      clusters <- list(mu = km$cluster, sigma= km$cluster, xi = km$cluster)
    }
    nclus <- lapply(clusters, function(x)length(unique(x)))
  }
  # Initialize parameters and latent variables
  alpha_cur <- init[["alpha"]];  theta_cur <- init[["theta"]];  tau_cur <- init[["tau"]]
  ...
  smp_alpha <- smp_theta <-  smp_tau <- rep(NA_real_, n_smp)
  ...
  prop_var[1,] <- c(tune_var[["alpha"]], tune_var[["theta"]],  tune_var[["tau"]])
  ...
  lK_cur <- make_kernel(obs_coord, nbasis, "smith", knot_coord, kern_bw = tau_cur)
  ...
  for(i in 1:n_iter){
    ...
    # Update kernel bandwidth (tau)
    tau_prop <- rnorm_prop(tau_cur, tune_var = prop_var[pos, "tau"])
    if((tau_prop > tau_tol)&(tau_prop <knot_rng[2])&(tau_prop > knot_rng[1])){
      lK_prop <- make_kernel(obs_coord, nbasis, "smith", knot_coord, kern_bw = tau_prop)
      lA_prop <- mklA(lK_prop, lB_cur, alpha_cur)
      qfuns_prop <- create_qlstabmix(alpha_cur, theta_cur, lK_prop)
      lz_prop <- gev2lsm(y, mu_cur, sigma_cur, xi_cur, qfuns_prop)
      if((!any(is.na(lz_prop)))&
         (log(runif(1)) <
         lpst_tau(tau_prop, lz_prop, y, lA_prop, lK_prop, alpha_cur, theta_cur, mu_cur, sigma_cur, xi_cur, tau_rng = knot_rng) -
         lpst_tau(tau_cur, lz_cur, y, lA_cur, lK_cur, alpha_cur, theta_cur, mu_cur, sigma_cur, xi_cur, tau_rng = knot_rng)
      )){
        tau_cur <- tau_prop
        lK_cur <- lK_prop
        lA_cur <- lA_prop
    ...
    prop_var[nxt_pos, "tau"] <- update_var(prop_var[pos, "tau"], acpt_rt = mean(acpt[,"tau"], na.rm = TRUE),
                                             opt_rt = opt_rt, gamma1 = gamma1)
  ...
  smp_mat <- cbind(smp_alpha, smp_theta, smp_tau, smp_locpar, smp_scalepar, smp_shapepar)
  colnames(smp_mat) <- c("alpha","theta","tau",locnames, scalenames, shapenames)
